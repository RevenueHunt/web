...
<div style="margin:32px 0;padding:24px 28px;background:#f8fafc;border-left:4px solid #16161D;border-radius:6px;">
  ...
  <ul style="list-style:none;padding:0;margin:0;">
    <li style="display:flex;gap:14px;padding:8px 0;font-size:15px;color:#0f172a;line-height:1.55;border-top:1px solid #e2e8f0;"><span style="flex-shrink:0;font-variant-numeric:tabular-nums;font-weight:700;color:#16161D;min-width:24px;">01</span><span>Where each metric lives in the RevenueHunt Quiz Builder (Responses, Analytics, Drop Off panels).</span></li>
    <li style="display:flex;gap:14px;padding:8px 0;font-size:15px;color:#0f172a;line-height:1.55;border-top:1px solid #e2e8f0;"><span style="flex-shrink:0;font-variant-numeric:tabular-nums;font-weight:700;color:#16161D;min-width:24px;">02</span><span>Platform benchmarks: 5.5% quiz-taker conversion (1 in 18), 10.6% at single results page, 11.0% at 9-12 questions, +11-15% AOV.</span></li>
    <li style="display:flex;gap:14px;padding:8px 0;font-size:15px;color:#0f172a;line-height:1.55;border-top:1px solid #e2e8f0;"><span style="flex-shrink:0;font-variant-numeric:tabular-nums;font-weight:700;color:#16161D;min-width:24px;">03</span><span>The fix for each metric when it lands below target, organised by issue and solution.</span></li>
    <li style="display:flex;gap:14px;padding:8px 0;font-size:15px;color:#0f172a;line-height:1.55;border-top:1px solid #e2e8f0;"><span style="flex-shrink:0;font-variant-numeric:tabular-nums;font-weight:700;color:#16161D;min-width:24px;">04</span><span>How to export raw response data for deeper analysis in Sheets or Excel.</span></li>
  </ul>
</div>

For the foundations that produce these metrics, see [how to build a successful product recommendation quiz](https://revenuehunt.com/how-to-build-a-successful-ecommerce-quiz/). For the mistakes that quietly drag the numbers down, see [product quiz mistakes ranked by industry data](https://revenuehunt.com/quiz-creation-mistakes-that-hurt-your-ecommerce-sales/).

**Table of contents:**

- [Where to find the data](#where-to-find-the-data)
- [What to analyse and how to improve?](#what-to-analyse-and-how-to-improve)
...
<p class="not-prose mt-2 mb-6 text-xs text-slate-500 leading-snug"><span class="font-semibold uppercase tracking-wider text-slate-600">Fig. 01</span> &nbsp;The four metrics that predict whether a product recommendation quiz moves revenue or quietly underperforms: completion rate, quiz-to-cart conversion, average order value, and recommendation precision. Each diagnoses a different failure mode in the funnel.</p>

## Where to find the data

The [Metrics panel](https://docs.revenuehunt.com/reference/quiz-builder/metrics/) has the same three sub-sections on every version of the RevenueHunt app (**Responses**, **Analytics**, **Drop-off**), but the location of the panel and the depth of order-level data differ between [Built for Shopify](/revenuehunt-built-for-shopify/) and the legacy / multi-platform versions.
...
These are the benchmarks to compare your own quiz against. If your conversion rate lands below 5%, the structural fixes in the table below recover most of the gap. If it lands above 8%, the rest of the leverage is in the [follow-up email sequence](https://revenuehunt.com/quiz-follow-up-emails/) and [discount placement](https://revenuehunt.com/product-quiz-discounts/).
...
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "ItemList",
  "name": "The four product quiz metrics that predict conversion",
  "description": "The four categories of metric available in the RevenueHunt Quiz Builder Analytics panel that together predict whether a product recommendation quiz moves revenue.",
  "numberOfItems": 4,
  "itemListElement": [
    {"@type": "ListItem", "position": 1, "name": "Customer Engagement", "description": "Completion rate, quiz starts and responses, per-question drop-off. The cleanest signal of whether the quiz is engaging and user-friendly. Healthy quizzes clear 70% completion; below 50% means the quiz is too long, too text-heavy or asking for personal data too early."},
    {"@type": "ListItem", "position": 2, "name": "Revenue", "description": "Conversion rate, number of orders, total orders value, average order value. Top-converting quizzes hit 10.6% conversion at a single results page; quiz orders typically run 11-15% larger AOV than non-quiz orders."},
    {"@type": "ListItem", "position": 3, "name": "Recommendations", "description": "Most-clicked and most-purchased products from the results page. Diagnoses recommendation precision: which products convert from which answer combinations."},
    {"@type": "ListItem", "position": 4, "name": "Customer Answers", "description": "Most-clicked answer choices and customer tags for segmentation. Reveals what customers actually want, drives the Klaviyo segments that compound revenue over time."}
  ]
}
</script>
...
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "HowTo",
  "name": "Diagnose and fix a product quiz that converts below 5%",
  "description": "The structural fixes for a product recommendation quiz that converts below the 10.6% top-performer benchmark, in priority order. Each step removes a specific friction source; apply in order until conversion reaches benchmark.",
  "totalTime": "PT2H",
  "supply": [
    {"@type": "HowToSupply", "name": "Access to the RevenueHunt Quiz Builder Analytics panel for baseline conversion rate"},
    {"@type": "HowToSupply", "name": "A connected Klaviyo account (or a target email platform)"}
  ],
  "step": [
    {"@type": "HowToStep", "name": "Collapse to a single results page", "text": "79% of top-converting quizzes use a single results page rather than splitting recommendations across multiple pages. Single results page lifts conversion to 10.6% on the platform benchmark. Edit the quiz to consolidate."},
    {"@type": "HowToStep", "name": "Tune to 6-12 questions, ideally 9-12", "text": "The 9-12 question sweet spot averages 11.0% completion-to-purchase on the platform. Shorter than 6 underspecifies the recommendation; longer than 12 erodes completion. Cut redundant questions; merge fields where possible."},
    {"@type": "HowToStep", "name": "Make email capture required", "text": "71% of high performers make email capture required and 75% of those collect email this way. Required email filters low-intent visitors and produces a higher-quality lead list. Edit the email question and toggle required."},
    {"@type": "HowToStep", "name": "Connect to Klaviyo", "text": "Segmented Klaviyo campaigns earn over 3x the revenue per recipient of generic sends and 1 in 5 quiz-attributed orders lands more than 30 days after the quiz. Connect Klaviyo and build tag-driven segments to capture this long-tail revenue."}
  ]
}
</script>
